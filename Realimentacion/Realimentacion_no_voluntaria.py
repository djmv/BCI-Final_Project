# -*- coding: utf-8 -*-
"""
Created on Fri Oct 27 15:42:37 2017

Este script corresponde a la realimentación No voluntaria del sujeto
Aquí el sujeto debe ejecutar la actividad que se le indique y se le calificará
...
class game(object):
    ...
    def draw_inst(self, text, color = VERDE, dw = 0, dh = 0):
        """Center text in window"""
        fw, fh = self.font.size(text) # fw: font width,  fh: font height
        surface = self.font.render(text, True, color)
        # // makes integer division in python3
    ...
    def concentration(self,t,timeOut):
        ...
        self.square_mov(3)
        self.draw_inst(str(g),(100,255,100))
        pygame.display.flip()
        #self.screen.blit(self.background, (0, 0))
        time.sleep(timeOut)
        d = self.getDataO(t)
    ...
    def square_mov(self,mov):
        centerX = (self.width/2) -50
        centerY = (self.height/2) - 50
        if mov == 1:
            for i in range(0,10):
                centerY -= self.r
                pygame.draw.rect(self.screen,VERDE, [centerX,centerY, 100, 100])
                pygame.display.flip()
                self.screen.blit(self.background, (0, 0))
                time.sleep(0.25)
        if mov == 2:
    ...
    def good(self):
        self.draw_inst("Bien :) ",(100,255,100))
        pygame.display.flip()
        #self.screen.blit(self.background, (0, 0))

    def bad(self):
        self.draw_inst("Mal :( ",(100,255,100))
        pygame.display.flip()
        #self.screen.blit(self.background, (0, 0))
   
    def relaxation(self,t,timeOut):
        self.square_mov(3)
        self.draw_inst("[+]",(100,255,100))
        pygame.display.flip()
        #self.screen.blit(self.background, (0, 0))
        time.sleep(timeOut)
        d = self.getDataO(t)
